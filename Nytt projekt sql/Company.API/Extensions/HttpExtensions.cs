...
namespace Company.API.Extensions
{
    public static class HttpExtensions
    {
        ...
        public static async Task<IResult> HttpDeleteAsyncMany<TReferenceEntity, TDto>(this IDbService db, TDto dto)
            where TReferenceEntity : class, IReferenceEntity
        {
            try
            {
                var result = db.Delete<TReferenceEntity, TDto>(dto);
                if (result && await db.SaveChangesAsync())
                    return Results.NoContent(); if (result && await db.SaveChangesAsync())
                    return Results.NoContent();
            }
            catch { }
